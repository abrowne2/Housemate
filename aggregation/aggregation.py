...
def performChoice(hbool, abool, seopath, budget, beds):
    ...
    if hbool is True and abool is True: #User does not care. Fetch both houses/apartments.
        haws = requests.get(('http://rent.com' + seopath + 'condos_houses_townhouses' + (('_max-price-' + budget) + '_' + beds + '-bedroom')), verify=False, timeout=10)
        cribz = requests.get(('http://rent.com' + seopath + 'apartments_condos_townhouses' + (('_max-price-' + budget) + '_' + beds + '-bedroom')), verify=False, timeout=10)
...
def rentPropertyTraversal(listIds, univLoc, usrDist):
    ...
    propJSON = json.loads(requests.get(('http://rent.com/account/myrent/listings.json?ids=' + listIds), verify=False, timeout=10).text,)
    ...
    for x in properties:
        propInfo = BeautifulSoup(requests.get(('http://rent.com' + x['url_path']), verify=False, timeout=10).text, "html.parser")
